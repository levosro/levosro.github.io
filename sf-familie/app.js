...
const subtitlu = document.getElementById('subtitle');
    ...
    function (value) {
      ...
      if (x != undefined) {
        ...
        for (let p = 0; p < texts.length; p++) {
          let text = texts[p];
          res = res + `<tr><td><span style="text-align: center;"><div><a href="./index.html?id=T${text.idChr}#${text.idChr}" id="CHR${text.idChr}">${text.title}</a></div><div id="chr${text.idChr}"></div><hr style="width:30%;"/></span></tr></td>`;
        }
        ...
        // res = res + '<ul>'
        res = res + `<div style="text-align: center;" class="dt"><b><a href="./index.html?id=C1.01" id="a1">Sfânta familie, sau critica criticii critice</a></b></div>`;
        res = res + `<ul class="partOl">`
        for (let j = 0; j < texts.length; j++) {
          let text = texts[j];
          res = res + `<li><a href="./index.html?id=T${text.idChr}" id="aT${text.idChr}">${text.title}</a></li>`;
          res = res + '<ul>'
          let hes = text.content.filter(item => item.includes('<h'))
          let h5 = text.content.filter(item => item.includes('<h5'))
          hes.shift()
          hes.forEach(element => {
            if (h5.includes(element)) {
              let indexH5 = h5.indexOf(element)
              let index = hes.indexOf(element)


              res = res + `<li><a href="./index.html?id=T${text.idChr}#${element.substring(element.indexOf('sigil'), element.indexOf('\"', element.indexOf('sigil')) + 1)}">${element.replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></li>`
              let final = indexH5 < h5.length - 1 ? hes.indexOf(h5[indexH5 + 1]) : hes.length;
              res = res + '<ul>'
              for (let l = index + 1; l < final; l++) {
                res = res + `<li><a href="./index.html?id=T${text.idChr}#${hes[l].substring(hes[l].indexOf('sigil'), hes[l].indexOf('\"', hes[l].indexOf('sigil')) + 1)}">${hes[l].replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></li>`
              }
              res = res + '</ul>'
            }
          });
        ...
        document.getElementById('download').addEventListener('click', function () {
          downloadFile('./Sfânta familie, sau critica criticii critice.epub', 'Sfânta familie, sau critica criticii critice.epub')
        })
...
function changeChapter(index) {
  ...
  // console.log(item)
  part.innerHTML = item.title.substring(0, item.title.indexOf('\.'))
  // subtitlu.innerHTML = item.subtitle
  contents.innerHTML = '';
  btnList = []
  let descriere = '';
  titlu.innerHTML = item.title.indexOf('\.') != -1 ? item.title.substring(item.title.indexOf('\.') + 2) : item.title;
  document.title = item.title.indexOf('\.') != -1 ? item.title.substring(item.title.indexOf('\.') + 2) : item.title;
  info.innerHTML = descriere;
  chTexts = ContentList(item.idCh);
  let contentString = ''
  for (let i = 0; i < chTexts.length; i++) {
    let expBtn = new ExpandedButton(chTexts[i].title.indexOf('\.') != -1 ? chTexts[i].title.substring(0, chTexts[i].title.indexOf('\.')) : chTexts[i].title, chTexts[i].idChr, i);
...
function generateTOC() {
  ...
  res = res + '<div id="toc">'
  res = res + '<menuitem><menuitem> <span class="heading"><a href="./index.html" id="A0.0"><i class="fa fa-book"></i> Sfânta familie, sau critica criticii critice</a></span> </menuitem>';
  for (let j = 0; j < texts.length; j++) {
    let text = texts[j];
    ...
    if (hes.length == 0) { res = res + `<menuitem> <span><a href="./index.html?id=T${text.idChr}#${text.idChr}" id="aCh${text.idChr}">${text.title}</a></span> </menuitem>`; }
    else {



      res = res + `<menuitem> <label for="left-menu-item-${j + 1}"><a href="./index.html?id=T${text.idChr}" id="a${text.idChr}">${text.title}</a></label> <input id="left-menu-item-${j + 1}" type="checkbox"> <nav> <main> <label class="menu-toggle" for="left-menu-item-${j + 1}"><span>&nbsp;</span></label> <menu>`;
      res = res + `<menuitem> <span class="heading"><a href="./index.html?id=T${text.idChr}" id="A${text.idChr}">${text.title}</a></span> </menuitem>`;
      hes.forEach(element => {
        if (h5.includes(element)) {
          let indexH5 = h5.indexOf(element)
          let index = hes.indexOf(element)
          ...
          if (final - index > 1) {
            res = res + `<menuitem> <label for="left-menu-item-${indexH5 + 1}"><a href="./index.html?id=T${text.idChr}#${element.substring(element.indexOf('sigil'), element.indexOf('\"', element.indexOf('sigil')) + 1)}">${element.replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></label> <input id="left-menu-item-${indexH5 + 1}" type="checkbox"> <nav> <main> <label class="menu-toggle" for="left-menu-item-${indexH5 + 1}"><span>&nbsp;</span></label> <menu>`;
            res = res + `<menuitem> <span class="heading"><a href="./index.html?id=T${text.idChr}#${element.substring(element.indexOf('sigil'), element.indexOf('\"', element.indexOf('sigil')) + 1)}">${element.replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></span> </menuitem>`;
            for (let l = index + 1; l < final; l++) {
              res = res + `<menuitem> <span> <a href="./index.html?id=T${text.idChr}#${hes[l].substring(hes[l].indexOf('sigil'), hes[l].indexOf('\"', hes[l].indexOf('sigil')) + 1)}">${hes[l].replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></span> </menuitem>`
            }

            res = res + `</menu></main></nav></menuitem>`
          }
          else {
            res = res + `<menuitem> <span> <a href="./index.html?id=T${text.idChr}#${element.substring(element.indexOf('sigil'), element.indexOf('\"', element.indexOf('sigil')) + 1)}">${element.replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></span> </menuitem>`
...
function openButton(text, btnX, i) {
  ...
  for (j = 0; j < hes.length; j++) {
    resString = resString + '\n' + `<li><a href="#${hes[j].substring(hes[j].indexOf('sigil'), hes[j].indexOf('\"', hes[j].indexOf('sigil')) + 1)}">${hes[j].replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></li>`;
  }
  resString = resString + '</ul>'

  for (j = 0; j < text.content.length; j++) {
    if (!text.content[j].includes(['\"marx\"', '\"marxy\"', '\"marxo\"', '\"engelsy\"', '\"engels\"', '\"marxengels\"'])) {
      if (!text.content[j].includes('<h'))
        resString = resString + '\n' + text.content[j];
      else {
        let start = text.content[j].substring(text.content[j].indexOf('<h') + 2, text.content[j].indexOf('<h') + 3)
        // console.log(start)
        resString = resString + '\n' + hChanger(text.content[j], start, 6);
      }
    }
  }
...
function hChanger(h, start, end) {
  return h.replace(new RegExp(`h${start}`, 'g'), `h${end}`)
}
