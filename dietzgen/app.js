  ...
  myPromise.then(
    function (value) {
      ...
      if (x != undefined) {
        ...
        // res = res + '<ul>'
        res = res + `<div style="text-align: center;" class="dt"><b><a href="./index.html?id=C1.01" id="a1">Esența muncii cerebrale a omului</a></b></div>`;
        res = res + `<ul class="partOl">`
        for (let j = 0; j < texts.length; j++) {
          let text = texts[j];
          let hes = text.content.filter(item => item.includes('<h5'))
        ...
        document.getElementById('download').addEventListener('click', function () {
          downloadFile('./Esența muncii cerebrale a omului.epub', 'Esența muncii cerebrale a omului.epub')
        })
...
function generateTOC() {
  ...
  res = res + '<div id="toc">'
  res = res + '<menuitem><menuitem> <span class="heading"><a href="./index.html" id="A0.0"><i class="fa fa-book"></i> Esența muncii cerebrale a omului</a></span> </menuitem>';
  for (let j = 0; j < texts.length; j++) {
    let text = texts[j];
    ...
    if (hes.length == 0) { res = res + `<menuitem> <span><a href="./index.html?id=T${text.idChr}#${text.idChr}" id="aCh${text.idChr}">${text.title}</a></span> </menuitem>`; }
      ...
      hes.forEach(element => res = res + `<menuitem> <span> <a href="./index.html?id=T${text.idChr}#${element.substring(element.indexOf('sigil'), element.indexOf('\"', element.indexOf('sigil')) + 1)}">${element.replace(/<br\/>/g, ' ').replace(/<[^>]*>/g, '').replace(/\[(\d)+\]/g, '')}</a></span> </menuitem>`)
